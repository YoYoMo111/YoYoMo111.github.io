...
Tween.prototype.lerp = function(t)
{
	...
	if (this.values[0] instanceof t.Vector3)
...
function Particle()
{
	...
	this.velocity     = new t.Vector3(); // units per second
	this.acceleration = new t.Vector3();
...
Particle.prototype.update = function(dt)
{
	...
	if ( this.colorTween.times.length > 0 )
	{
		...
		this.color = new t.Color().setHSL( colorHSL.x, colorHSL.y, colorHSL.z );
	}
...
function ParticleEngine()
{
	...
	this.positionBase   = new t.Vector3();
	// cube shape data
	this.positionSpread = new t.Vector3();
	// sphere shape data
	this.positionRadius = 0; // distance from base at which particles start
	
	this.velocityStyle = Type.CUBE;	
	// cube movement data
	this.velocityBase       = new t.Vector3();
	this.velocitySpread     = new t.Vector3(); 
	// sphere movement data
	//   direction vector calculated using initial position
	this.speedBase   = 0;
	this.speedSpread = 0;
	
	this.accelerationBase   = new t.Vector3();
	this.accelerationSpread = new t.Vector3();	
	...
	// store colors in HSL format in a t.Vector3 object
	// http://en.wikipedia.org/wiki/HSL_and_HSV
	this.colorBase   = new t.Vector3(0.0, 1.0, 0.5); 
	this.colorSpread = new t.Vector3(0.0, 0.0, 0.0);
	...
	this.blendStyle = t.NormalBlending; // false;
	...
	this.particleMaterial = new t.ShaderMaterial( 
	{
		...
		blending: t.NormalBlending, depthTest: true,
		
	});
	this.particleMesh = new t.Mesh();
}
...
ParticleEngine.prototype.setValues = function( parameters )
{
	...
	this.particleMesh = new t.ParticleSystem();
}
...
ParticleEngine.prototype.createParticle = function()
{
	...
	if (this.positionStyle == Type.SPHERE)
	{
		...
		var vec3 = new t.Vector3( r * Math.cos(t), r * Math.sin(t), z );
		particle.position = new t.Vector3().addVectors( this.positionBase, vec3.multiplyScalar( this.positionRadius ) );
	}
	...
	{
		var direction = new t.Vector3().subVectors( particle.position, this.positionBase );
	...
	particle.color = new t.Color().setHSL( color.x, color.y, color.z );
...
ParticleEngine.prototype.initialize = function()
{
	...
	if ( this.blendStyle != t.NormalBlending) 
		this.particleMaterial.depthTest = false;
	
	this.particleMesh = new t.ParticleSystem( this.particleGeometry, this.particleMaterial );
